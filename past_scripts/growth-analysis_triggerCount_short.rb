...
if ARGV[4]==nil then
  puts "Usage: ruby growth-analysis_lightcurve_short.rb <fits file> <adc channel> <bin size (sec)> <lower channel> <upper channel> <start unixtime> <duration>"
  exit 1
end
...
for i in 0..channelIndex-2
  ...
  (deltaTriggerCount-1).times do
    hist.Fill(unixTimeChannel[i].to_f-startTime)
  end
end
